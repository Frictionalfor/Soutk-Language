...
class SoutkInterpreter:
    ...
    def init_math_functions(self):
        """Initialize built-in math and string functions"""
        self.math_functions = {
            'sqrt': math.sqrt,
            'sin': math.sin,
            'cos': math.cos,
            ...
            'split': lambda x, d=" ": x.split(d) if isinstance(x, str) else str(x).split(d),
            'contains': lambda x, s: s in x if isinstance(x, str) else str(x).find(str(s)) != -1
        }
    ...
    def error(self, message):
        ...
        print(f"ERROR  Line {self.line_number}: {message}")
    ...
    def handle_file_operations(self, line):
        ...
        if scroll_match:
            ...
            try:
                ...
                print(f"Scrolled  Scrolled '{filename}' into '{var_name}'")
        ...
        if inscribe_match:
            ...
            try:
                ...
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(str(data))
                print(f"Inscribed  Inscribed data into '{filename}'")
            except Exception as e:
        ...
        if append_match:
            ...
            try:
                ...
                with open(filename, 'a', encoding='utf-8') as f:
                    f.write(str(data) + '\n')
                print(f"Inscribed  Appended data to '{filename}'")
            except Exception as e:
    ...
    def handle_module_operations(self, line):
        ...
        if invoke_match:
            ...
            try:
                ...
                print(f"Invoked  Invoked module '{filename}' as '{module_name}'")
            except FileNotFoundError:
    ...
    def handle_class_operations(self, lines, i):
        ...
        if enchant_match:
            ...
            while j < len(class_body):
                ...
                if body_line.startswith("spell construct") or body_line.startswith("forge construct") or body_line.startswith("construct"):
                    construct_match = re.search(r'(?:spell\s+|forge\s+spell\s+|forge\s+)?construct\s*\((.*?)\)\s*[{:]', body_line)
                    if construct_match:
                ...
                elif body_line.startswith("spell ") or body_line.startswith("forge spell "):
                    spell_match = re.search(r'(?:spell\s+|forge\s+spell\s+)(\w+)\s*\((.*?)\)\s*[{:]', body_line)
                    if spell_match:
            ...
            print(f"Enchanted  Enchanted class '{class_name}'")
    ...
    def eval_expr(self, expr):
        ...
        attribute_access_pattern = r'([a-zA-Z_]\w*)\.([a-zA-Z_]\w*)\b(?!\()'
        ...
        def process_invoke_in_expr(expr):
            ...
            while changed:
                ...
                pos = 0
                while pos < len(expr):
                    match = re.search(invoke_pattern, expr[pos:])
                    if not match:
                        break
                    
                    start = pos + match.start()
                    func_name = match.group(1)
                    paren_pos = pos + match.end() - 1
                    ...
                    for j in range(paren_pos + 1, len(expr)):
                        char = expr[j]
                    ...
                    args_str = expr[paren_pos + 1:end - 1]
                    restored_args = args_str
                    for k, literal in enumerate(string_literals):
                        restored_args = restored_args.replace(f"__STRING_{k}__", literal)
                    
                    try:
                        result = self.call_function(func_name, restored_args)
                        replacement = str(result)
                    except Exception as e:
                        replacement = f"ERROR_{str(e)}"
        ...
        def process_cast_in_expr(expr):
            ...
            while changed:
                ...
                pos = 0
                while pos < len(expr):
                    match = re.search(cast_pattern, expr[pos:])
                    if not match:
                        break
        ...
        expr = process_cast_in_expr(expr)
        ...
        # Handle invoke function calls in expressions (before string concatenation)
        invoke_pattern = r'invoke\s+(\w+)\s*\((.*?)\)'
    ...
    def call_function(self, func_name, args_str):
        ...
        if func_name in self.math_functions:
            ...
            if args_str.strip():
                ...
                args = [self.eval_expr(arg) for arg in args]
            ...
            if bind_match:
                ...
                if dict_name in self.data_structures:
                    ...
                    if isinstance(ds, SoutkGrimoire):
                        ...
                        ds.bind(key, value)
                        print(f"Bound  Bound '{key}' = '{value}' in grimoire '{dict_name}'")
                    else:
        ...
        if command == "push":
            ...
            if stack_name in self.data_structures:
                ...
                if isinstance(ds, SoutkStack):
                    ds.push(value)
                    print(f"Pushed  Pushed '{value}' to stack '{stack_name}'")
                else:
            ...
            if stack_name in self.data_structures:
                ...
                if isinstance(ds, SoutkStack):
                    ...
                    if value is not None:
                        print(f"Popped  Popped '{value}' from stack '{stack_name}'")
                    else:
            ...
            if stack_name in self.data_structures:
                ...
                if isinstance(ds, SoutkStack):
                    value = ds.peek()
                    if value is not None:
                        print(f"Top  Top of stack '{stack_name}': '{value}'")
                    else:
                        print(f"Stack '{stack_name}' is empty")
                else:
                    self.error(f"'{stack_name}' is not a stack")
            ...
            if stack_name in self.data_structures:
                ...
                if isinstance(ds, SoutkStack):
                    items = ds.show()
                    print(f"Stack  Stack '{stack_name}': {items}")
                else:
                    self.error(f"'{stack_name}' is not a stack")
            else:
                self.error(f"Stack '{stack_name}' not found")
        ...
        # QUEUE commands
            ...
            if queue_name in self.data_structures:
                ...
                if isinstance(ds, SoutkQueue):
                    ds.enqueue(value)
                    print(f"Enqueued  Enqueued '{value}' to queue '{queue_name}'")
                else:
            ...
            if queue_name in self.data_structures:
                ...
                if isinstance(ds, SoutkQueue):
                    value = ds.dequeue()
                    if value is not None:
                        print(f"Dequeued  Dequeued '{value}' from queue '{queue_name}'")
                    else:
            ...
            if queue_name in self.data_structures:
                ...
                if isinstance(ds, SoutkQueue):
                    value = ds.front()
                    if value is not None:
                        print(f"Top  Front of queue '{queue_name}': '{value}'")
                    else:
                        print(f"Queue '{queue_name}' is empty")
                else:
                    self.error(f"'{queue_name}' is not a queue")
            ...
            if queue_name in self.data_structures:
                ...
                if isinstance(ds, SoutkQueue):
                    items = ds.show()
                    print(f"Queue  Queue '{queue_name}': {items}")
                else:
                    self.error(f"'{queue_name}' is not a queue")
            else:
                self.error(f"Queue '{queue_name}' not found")
        ...
        # LINKED LIST commands
            ...
            if list_name in self.data_structures:
                ...
                if isinstance(ds, SoutkLinkedList):
                    ds.link(value)
                    print(f"Linked  Linked '{value}' to list '{list_name}'")
                else:
            ...
            if list_name in self.data_structures:
                ...
                if isinstance(ds, SoutkLinkedList):
                    if ds.unlink(value):
                        print(f"Unlinked Unlinked '{value}' from list '{list_name}'")
                    else:
            ...
            if list_name in self.data_structures:
                ...
                if isinstance(ds, SoutkLinkedList):
                    if ds.insert_after(after_value, new_value):
                        print(f"Linked  Inserted '{new_value}' after '{after_value}' in list '{list_name}'")
                    else:
            ...
            if list_name in self.data_structures:
                ...
                if isinstance(ds, SoutkLinkedList):
                    items = ds.traverse()
                    print(f"Linked  List '{list_name}': {' -> '.join(map(str, items))}")
                else:
                    self.error(f"'{list_name}' is not a linked list")
            else:
                self.error(f"Linked list '{list_name}' not found")
            ...
            if ds_name in self.data_structures:
                ...
                print(f"Stats '{ds_name}' is {'empty' if is_empty else 'not empty'}")
            else:
    ...
    def find_function_end(self, lines, start):
        ...
        while i < len(lines):
            if '{' in lines[i]:
                line = lines[i]
                brace_pos = line.find('{')
                close_pos = line.find('}', brace_pos + 1)
                if close_pos != -1:
                    return i + 1
                depth = 1
                i += 1
    ...
    def parse_function_body(self, lines, start):
        ...
        while i < len(lines):
            if '{' in lines[i]:
                depth = 1
                # Check if opening and closing brace are on same line
                line = lines[i]
                brace_pos = line.find('{')
                close_pos = line.find('}', brace_pos + 1)
                if close_pos != -1:
                    body_content = line[brace_pos + 1:close_pos].strip()
                    if body_content:
            ...
            try:
                ...
                if line.startswith("forge spell"):
                    ...
                    if spell_match:
                        ...
                        body, _ = self.parse_function_body(lines, i)
                ...
                if "conjure" in line:
                    ...
                    if conjure_match:
                        ...
                        if class_name in self.classes:
                            ...
                            print(f"Enchanted  Conjured {class_name} object '{var_name}'")
                        else:
                ...
                # TRANSFORM - Variable assignment
                    ...
                    if '=' in transform_line:
                        ...
                        if "conjure" in expr:
                            conjure_match = re.match(r'\s*conjure\s+(\w+)\s*\((.*?)\)', expr)
                            if conjure_match:
                                class_name = conjure_match.group(1)
                                args_str = conjure_match.group(2)
                                if class_name in self.classes:
                                    class_def = self.classes[class_name]
                                    obj = SoutkObject(class_def, self)
                                    if class_def.constructor and args_str.strip():
                                        args = [self.eval_expr(arg.strip()) for arg in args_str.split(',')]
                                    else:
                                        args = []
                                    old_variables = self.variables.copy()
                ...
                # OBJECT METHOD CALL - obj.method(args)
                    ...
                    if method_match:
                        ...
                        if obj_name in self.variables:
                            obj = self.variables[obj_name]
                            if isinstance(obj, SoutkObject):
                                args = [self.eval_expr(arg.strip()) for arg in args_str.split(',')] if args_str.strip() else []
                        ...
                        self.error("Invalid object method call syntax")
                ...
                # LOOP - Enhanced loop support
                    ...
                    if loop_match:
                        ...
                        while i < len(lines):
                            ...
                            if depth > 0:
                                loop_body.append(body_line.strip())
                            i += 1
                    ...
                    if if_match:
                        ...
                        while i < len(lines):
                            ...
                            if depth > 0:
                                if_body.append(body_line.strip())
                            i += 1
                        ...
                        if i < len(lines) and lines[i].strip().startswith("else"):
                            ...
                            if '{' in else_line:
                                ...
                                while i < len(lines):
                                    ...
                                    if depth > 0:
                                        else_body.append(body_line.strip())
                                    i += 1
                            else:
                                i += 1
                        
                        try:
                            condition_met = self.eval_expr(condition)
                            ...
                            self.error(f"Condition error: {str(e)}")
                    else:
                        self.error("Invalid if syntax")
                    continue
                
                elif line.startswith("this."):
                    this_line = line[5:].strip()
                    if '=' in this_line:
                        attr_name, expr = this_line.split('=', 1)
                        attr_name = attr_name.strip().rstrip(';')
                        expr = expr.strip(' ;')
                        if attr_name.startswith("transform "):
                            attr_name = attr_name[len("transform "):].strip()
                        if 'this' in self.variables and isinstance(self.variables['this'], SoutkObject):
                            value = self.eval_expr(expr)
                            if isinstance(value, str) and value.startswith('"') and value.endswith('"'):
                                value = value[1:-1]
                            self.variables['this'].set_attribute(attr_name, value)
                        else:
                            self.error("Cannot use 'this' outside of an object method")
                    else:
                        self.error("Invalid this syntax")
...
def main():
    ...
    try:
        ...
        print(f"Running Running Ultimate Soutk program: {filename}")
        print("=" * 50)
        ...
        print("=" * 50)
        print("Program completed Program completed successfully!")
        
    except Exception as e:
        print(f"Fatal error Fatal error: {str(e)}")
